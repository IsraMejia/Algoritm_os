...
public class ExamenHilos {
   ...
   public static void main(String[] args) {
        ...
        Modificador mensaje1 = new Modificador(matrizAfluencia[0]);
        Modificador mensaje2 = new Modificador(matrizAfluencia[1]);
        
        
        CConsumidor consumidor1 = new CConsumidor(mensaje1, "Jardín Botánico");
        CConsumidor consumidor2 = new CConsumidor(mensaje2, "Pabellón  Nacional  de  la Biodiversidad  ");
        consumidor1.start();
        consumidor2.start();
        ...
        for (int lugar = 0; lugar < numLugares; lugar++) {
            if(lugar==0){
                for (int i = 0; i < 2; i++) {
                    //LectorBoletos lector = new LectorBoletos(lugar, matrizAfluencia, mensaje1);
                    //lector.start();
                    new LectorBoletos(lugar, matrizAfluencia, mensaje1, i, "Jardín Botánico").start();
                }
            }
            else{
                for (int i = 0; i < 2; i++) {
                    new LectorBoletos(lugar, matrizAfluencia, mensaje2, i, "Pabellón  Nacional  de  la Biodiversidad  ").start();
                }
            }
